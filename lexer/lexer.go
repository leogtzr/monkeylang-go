...
type Lexer struct {
	...
	readPosition int  // current reading position in input (after current char)  always points to the next char.
...
func (l *Lexer) readChar() {
	if l.readPosition >= len(l.input) { // check for end of file or "we haven't read anything yet"
		l.ch = 0
	} else {
